...
> 1.0 构建版本；Claude Code 部分仍缺少充分测试，当前实现不成熟，请勿用于关键工作流
...
当前版本：`1.0`。项目尚未发布到 JetBrains Marketplace，目前通过源码运行或安装构建产物使用。Claude Code 相关功能尚未经过充分测试，稳定性和兼容性仍不成熟；建议优先使用 Codex 渠道，并在非关键项目中验证 Claude Code 工作流。

## 当前能力

- 原生 Codex 会话创建、恢复、历史和流式响应，以及 Claude Code CLI 会话创建、恢复、续接、流式响应和工具状态展示；Claude 与 Codex 会话按渠道和页签隔离，切换渠道时会在当前页签自动开启对应的新会话，另一渠道的运行中任务不会阻止切换或配置
...
- 自定义供应商启用前必须成功读取非空模型目录；Codex app-server 和 Claude 模型接口均会自动读取全部分页。插件优先恢复该供应商上次选择且仍有效的模型，否则选择接口首项；模型选择器可为当前供应商添加自定义模型并在顶部独立分组显示，展开长列表时会自动定位并高亮当前模型
...
- AI 回复进行中仍可发送消息；普通发送和发送快捷键会按当前会话顺序排队，GPT 渠道也可通过独立的“立即引导”按钮将输入追加到当前回合
- Codex 输入栏可直接选择只读、工作区或完全访问沙箱；审批策略与文件沙箱独立，全自动审批会开放命令网络访问，选择“全自动 + 完全访问”时不再限制命令审批、网络或文件范围
...
- 文本、文件引用与图片输入，支持编辑器选区上下文、项目树拖拽文件引用与图片附件；聊天输入框由 ProseMirror 结构化文档管理，文件引用是带稳定 ID 的内联原子节点，可按一个逻辑位置跨越、选择、删除、撤销、重做、拖动排序和右键剪切；输入 `@` 可搜索工作区实际文件并异步插入当前光标位置，包括 Rider 未纳入模块索引的 Unreal `Content` 资源，候选项继续显示项目相对路径；粘贴“`@绝对路径的成员描述`”或由 Markdown 行内代码包裹的同类内容时，会将有效文件路径转换为标签并保留后续描述，跨 IDE 窗口复制、剪切或粘贴混合内容时仍按原顺序恢复文件标签
...
- AI 回复中的本地文件路径链接支持直接跳转到 JetBrains 编辑器对应的文件、行和列，并兼容 Windows 盘符路径；右键链接可在系统资源管理器中定位文件，避免依赖系统默认打开程序
- 修改捕获栏由 Conversation ChangeSet 驱动：只有 Provider 明确报告或 Claude 文件 Hook 预先声明的路径才会进入列表；首次触碰保存存在/缺失基线，Codex 补丁只有在 after 侧与实时文件匹配时才会建立基线，同一回合的最新累计补丁可纠正早期临时基线；后续用户编辑从 JetBrains Document/VFS 或磁盘实时读取，并通过批量 VFS 事件只刷新已跟踪路径；新增/删除行数按基线到当前内容的逐行净差异统计，多处修改不会把中间未变内容重复计入；悬浮或聚焦文件名时会紧贴文件名上方显示完整项目相对路径，右键可在系统文件管理器中定位文件；Diff 页签保持单文件名，同一会话中重复打开同一文件时会定位到已有标签，接受、撤销或结束会话时同步关闭失效 Diff
...
- 左右方向键每次跨过一个文件标签并持续向两端移动，Shift 选择、Backspace/Delete、Home/End 和普通文本导航保持一致；输入框不再使用零宽字符或可编辑光标占位节点，文件标签高度与输入文字行高保持一致并与文字中线对齐，两侧均为光标保留透明间隔，空输入框提示也不会参与光标排版
...
- 设置在异步任务启动前固化为不可变快照，受限选项使用枚举并通过一次性 schema migration 兼容旧配置；项目文件搜索遍历工作区实际文件并跳过依赖、缓存和构建产物目录，所有工作区路径统一经过同一边界策略校验
...
自定义供应商的认证凭据仅保存到 JetBrains PasswordSafe，供应商列表和插件 XML 不保存明文凭据。切换 Claude / GPT 渠道时会在当前页签清空原对话并开启目标渠道的新会话；历史页按当前会话渠道筛选，Claude 历史读取当前项目的 Claude Code JSONL，Codex 历史展示当前项目下的 Codex 交互会话，两个渠道不会相互混入。修改同一渠道的供应商配置后，旧会话仍会阻止使用新配置继续发送，避免把上下文误发到另一个接口。
...
Windows 也可以使用一键打包脚本；脚本可从任意目录调用，成功后会输出发行包路径、文件大小和 SHA-256：
...
- Claude Code 部分当前缺少充分测试，功能成熟度和稳定性均未达到 Codex 渠道水平，不建议用于关键项目或不可恢复的修改。
