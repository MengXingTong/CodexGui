...
当前开发版本：`0.3.4`。项目尚未发布到 JetBrains Marketplace，目前只能从源码运行或自行构建插件包。
...
- 文本、文件引用与图片输入，支持编辑器选区上下文、项目树拖拽文件引用与图片附件；输入 `@` 可稳定搜索项目文件并插入文件标签，搜索过程不会打断聊天框文字选择；文件标签按输入位置发送给 Codex，并统一传递为 `@` 加完整绝对路径，也可直接拖动排序、选择、复制或剪切
- 用户/项目 Skills 导入、启停和 Codex 自动发现
- 会话内命令、计划、MCP 与文件修改事件展示；命令以单行省略形式显示，并用红绿圆点标识执行结果
- 基于 Codex app-server 文件修改 diff 的修改捕获栏、IDE Diff、逐文件/全部接受或撤销；不扫描整个工作区
