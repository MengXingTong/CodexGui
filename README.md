...
- 自定义供应商启用前必须成功读取非空模型目录；Codex app-server 和 Claude 模型接口均会自动读取全部分页。插件优先恢复该供应商上次选择且仍有效的模型，否则选择接口首项；模型选择器可为当前供应商添加自定义模型，并在顶部独立分组显示
...
- 文本、文件引用与图片输入，支持编辑器选区上下文、项目树拖拽文件引用与图片附件；聊天输入框由 ProseMirror 结构化文档管理，文件引用是带稳定 ID 的内联原子节点，可按一个逻辑位置跨越、选择、删除、撤销、重做和拖动排序；输入 `@` 可搜索项目文件并异步插入原位置，粘贴“`@绝对路径的成员描述`”时会将有效文件路径转换为标签并保留后续描述，跨 IDE 窗口复制、剪切或粘贴混合内容时仍按原顺序恢复文件标签
